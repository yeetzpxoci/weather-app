...
async function getHourlyWeatherData(lat, lon) {
    const response = await fetch('https://pro.openweathermap.org/data/2.5/forecast/hourly?lat=' + lat + '&lon=' + lon + '&appid=42d9bbce104de377e00f785f9b102383&units=metric', { mode: 'cors' });
    const weatherData = await response.json();
    return weatherData;
}

async function getDailyWeatherData(lat, lon) {
    const response = await fetch('api.openweathermap.org/data/2.5/forecast/daily?lat=' + lat + '&lon=' + lon + '&appid=42d9bbce104de377e00f785f9b102383&units=metric', { mode: 'cors' });
    const weatherData = await response.json();
    return weatherData;
}
