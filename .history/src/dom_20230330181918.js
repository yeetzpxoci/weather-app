...
function showHourlyWeatherDetails(weatherArray, hoursArray) {
    for (let i = 0; i < 3; i++) {
        ...
        switch (i) {
            case 1:
                hourlyWeatherContainer.style.transform = "translateX(2000px)"
                hourlyWeatherContainer.style.position = "absolute"
                break;
            case 2:
        ...
        for (let j = 0; j < 5; j++) {
            ...
            dayHour.innerText = hoursArray[i + j];

            // create second child image with class "weather-icon-day-hour"
            const weatherIconDayHour = document.createElement("img");
            weatherIconDayHour.classList.add("weather-icon-day-hour");
            hourlyWeatherInfo.appendChild(weatherIconDayHour);
            weatherIconDayHour.src = "https://openweathermap.org/img/wn/" + weatherArray[i + j].icon.slice(0, -1) + 'd' + "@2x.png";
            ...
            degreeSpan.innerText = weatherArray[i + j].temp + '°';
