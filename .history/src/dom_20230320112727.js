import { celsiusToFahrenheit, getDays } from "./converter";
...
celsiusConverterButton.addEventListener("click", function () {
    if (!celsiusActive) {
        ...
        days.forEach(x => x.innerHTML = fahrenheitToCelsius(parseInt(x.innerHTML)));
...
fahrenheitConverterButton.addEventListener("click", function () {
    if (celsiusActive) {
        const days = document.querySelectorAll(".degree-daily");
        days.forEach(x => x.innerHTML = celsiusToFahrenheit(parseInt(x.innerHTML)));
